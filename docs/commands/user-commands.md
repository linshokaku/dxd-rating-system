...
ランキング確認結果を受け取るための private thread を作成する。
...
- `レート戦ランキング` チャンネル配下に、ランキング確認用の private thread を 1 つ作成する。
- private thread の親チャンネルは、このコマンドをどのチャンネルで実行したかには依存しない。
- 作成した thread には、少なくとも実行ユーザー、admin、Bot を参加させる。
- Bot は、作成した `thread_id` を実行ユーザーの最新のランキング確認 thread として一時的に紐づける。
...
- ランキング表示用チャンネルが見つからない:
  - `ランキング表示用チャンネルが見つかりません。管理者に確認してください。`
- 内部エラー:
  - `ランキング確認用スレッドの作成に失敗しました。管理者に確認してください。`

#### 備考

- 作成直後の案内文と thread の可視性ルールは [../ui/leaderboard_thread.md](../ui/leaderboard_thread.md) を参照する。
- ランキング表示用チャンネルの用途と権限は [../ui/registered_channels.md](../ui/registered_channels.md) を参照する。
...
- 1 ページあたり 20 件ずつ取得する。
- 表示内容は、実行ユーザーに現在紐づいているランキング確認用 private thread に送る。
...
- 実行ユーザーに紐づくランキング確認用 thread が存在しない:
  - `先に /leaderboard_thread を実行してください。`
- 紐づけ先の thread が存在しない、または利用できない:
  - `ランキング確認用スレッドが見つかりません。先に /leaderboard_thread を実行してください。`
- 指定したページに表示対象が存在しない:
...
- thread への表示形式は [../ui/leaderboard_thread.md](../ui/leaderboard_thread.md) を参照する。
