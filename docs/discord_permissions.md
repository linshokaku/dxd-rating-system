...
| 権限 | 用途 | 理由 |
| --- | --- | --- |
| `Manage Channels` | `/admin_setup_custom_ui_channel`、`/admin_setup_ui_channels`、`/admin_resetup_ui_channel`、`/admin_cleanup_ui_channels`、`/admin_teardown_ui_channels` | 管理対象チャンネルの作成、permission overwrite 設定、再セットアップ、cleanup、削除を行うため |
...
- `/admin_setup_custom_ui_channel`、`/admin_setup_ui_channels`、`/admin_resetup_ui_channel`、`/admin_cleanup_ui_channels`、`/admin_teardown_ui_channels` は、事前に判定できた不足権限をレスポンスへ表示してよい。
