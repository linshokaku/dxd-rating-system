...
| --- | --- | ---: | ---: | ---: | --- |
| `1v1` | 1 人 vs 1 人 | 1 | 2 | 4 | 4 人集めて 2 マッチ同時に作る |
| `2v2` | 2 人 vs 2 人 | 2 | 1 | 4 | 4 人集めて 1 マッチ作る |
| `3v3` | 3 人 vs 3 人 | 3 | 1 | 6 | 6 人集めて 1 マッチ作る |
...
- `1v1` のマッチは `1v1` の保持値だけを更新する
- `2v2` のマッチは `2v2` の保持値だけを更新する
- `3v3` のマッチは `3v3` の保持値だけを更新する
...
- `1v1` は `batch_size = 2` とし、1 回のマッチング試行で 2 マッチを同時に生成する
- `2v2` と `3v3` は `batch_size = 1` とし、1 回のマッチング試行で 1 マッチだけ生成する
...
`2v2` と `3v3` の `batch_size > 1` を導入する場合は、複数マッチ同時最適化は行わず、以下の逐次構築方式を採用する。

1. バッチ内の全プレイヤーを、対象フォーマットのレート降順でソートする
2. 先頭から、`2v2` なら 4 人ずつ、`3v3` なら 6 人ずつ切り出してマッチを作る
3. 各マッチの内部では、そのマッチに含まれるプレイヤーだけを対象に、期待勝率が `0.5` に最も近くなるようチーム分けする
...
- `match_id` 単位でマッチ進行を管理する
- `1v1` の 1 バッチ 2 マッチは、2 件の `matches` 行として持つ
