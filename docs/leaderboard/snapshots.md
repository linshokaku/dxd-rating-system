...
- 参照側で重い再計算や複雑な時系列復元を行わずに済むようにする

## 基本方針

- snapshot は Bot の正史データではなく、ランキング表示のための派生データとする
- snapshot は Bot と同じ PostgreSQL に保持してよい
- ランキング参照機能は snapshot を read-only で参照してよい
- snapshot は日次で作成する
- 一度作成した過去 snapshot は、admin によるマッチ結果補正があっても更新しない
...
admin によるマッチ結果補正が行われた場合でも、既存の snapshot は更新しない。
...
ランキング参照機能は、少なくとも以下の 2 系統を読めればよい。

- 現在ランキング用の、稼働中シーズンの `player_format_stats`
- 過去比較用の `leaderboard_snapshots`

これにより、参照側は DB への read 権限だけで現在ランキングと順位比較を組み立てられる。
