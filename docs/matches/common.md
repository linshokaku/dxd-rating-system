...
`1v1`、`2v2`、`3v3` に共通するマッチ進行、親決定、勝敗報告、承認、結果確定、ペナルティ反映を定める。
...
各マッチは `match_format` を持ち、以下の値を導く。
...
- 以後のマッチタイマーはすべて `parent_decided_at` を基準に計測する
- 親決定後の部屋立てと部屋 ID 共有は運用で行い、Bot では管理しない

### 4. マッチ進行

- 部屋立て、各試合の開始、ゲーム間インターバル、各試合の終了は運用で進行する
- Bot はマッチ内の各試合の進行そのものは管理しない
- ただし、勝敗報告の受付開始時刻、締切時刻、承認期間はタイマーで管理する

### 5. 勝敗報告

- 勝敗報告は、そのマッチ参加者のみが行える
...
- Bot は所属チームに応じて内部結果へ正規化する
  ...
  - `引き分け` と `無効マッチ` は両チーム共通
- `無効マッチ` だけは `match_created_at` の時点から受け付ける
...
- `無効マッチ` は `match_created_at` の時点から受け付ける
...
1. 誰も報告していない場合は、無効マッチを採用する
2. 親が最新報告を行っており、その正規化後の報告先が同票候補の中に含まれていれば、親の報告を採用する
3. それでも決まらない場合は、無効マッチとし、admin への連絡を行う
...
- マッチに紐づく操作コマンドは、少なくとも `match_id` を引数に取る
- `match_id` から `match_format` を解決できるため、マッチ操作コマンドに `match_format` の追加引数は不要とする
