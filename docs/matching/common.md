...
- 通常ユーザー操作でも開発者コマンド操作でも、タイマー通知は、その通知先コンテキストに保存された最新の `channel_id` へ通常のテキストメッセージとして送る。
- 通知メッセージの先頭には、対象ユーザーへの mention を付ける。
- `レート戦マッチング` チャンネルの UI から参加した場合も、参加ボタン押下時の interaction が発生した channel を最新通知先として扱う。
- `/join` を起点にした待機でも `/present` 後の再待機でも、同じ通知方式を使う。
