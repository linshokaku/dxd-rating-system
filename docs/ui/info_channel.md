...
3. Bot は、押下された button に対応する `command_name` を使って `/info_thread` と同じ業務処理を実行する。
4. 成功時は、`レート戦情報` チャンネル配下に情報確認用の private thread を 1 つ作成する。
5. 作成した thread には、少なくとも実行ユーザー、admin、Bot を参加させる。
...
- 情報確認用チャンネルが見つからない:
  - ephemeral なテキストメッセージで `情報確認用チャンネルが見つかりません。管理者に確認してください。` を返す。
- 内部エラー:
  - ephemeral なテキストメッセージで `情報確認用スレッドの作成に失敗しました。管理者に確認してください。` を返す。
