...
- このチャンネルには、以下の 5 通の常設メッセージを設置する。
  1. 遊び方メッセージ
  2. 参加状況メッセージ
  3. `1v1` 用の参加 panel
  4. `2v2` 用の参加 panel
...
- 登録済みユーザーは、参加したい形式に対応する 3 通目から 5 通目の panel で階級を選び、参加ボタンからマッチングキューへ参加できる。
- 現時点の各参加 panel は、対応する `match_format` 固定の階級プルダウンと参加ボタンのみで構成する。
