...
- マッチングチャンネル配下で Bot が作成する private thread の用途。
...
### `matchmaking_channel` / `matchmaking_news_channel` / `system_announcements_channel`
...
- つまり登録済みユーザーも、これら 3 チャンネルでは閲覧と UI 操作だけを行い、通常メッセージ送信や thread 作成は行わない。
...
- 各アナウンスメッセージには、その試合への観戦 button を設置する。
...
- 未登録ユーザーは、`matchmaking_channel`、`matchmaking_news_channel`、`system_announcements_channel` には入れない。
- 登録完了後は、登録済みユーザー向け閲覧権限の対象となり、`matchmaking_channel`、`matchmaking_news_channel`、`system_announcements_channel` を閲覧できる。
