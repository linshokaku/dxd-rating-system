...
| `leaderboard_channel` | `レート戦ランキング` | `/leaderboard_thread` で作成するランキング確認 thread の親チャンネル |
| `system_announcements_channel` | `レート戦アナウンス` | admin からのシステム告知 |
| `admin_contact_channel` | `運営連絡・フィードバック` | admin への連絡、問い合わせ、フィードバック |

## 共通権限ルール

### `matchmaking_channel` / `matchmaking_news_channel` / `leaderboard_channel` / `system_announcements_channel`
...
- このチャンネルには、ランキング確認 thread の使い方を示す常設メッセージを 1 つ以上設置する。
- 登録済みユーザーは、このチャンネルを閲覧できる。
- 一般ユーザーはこのチャンネルへ通常メッセージを送らない。
- 一般ユーザーは public thread を作成しない。
- 一般ユーザーは private thread を作成しない。
- `/leaderboard_thread` が成功した場合、Bot はこのチャンネル配下にランキング確認用の private thread を作成する。
- ランキング確認 thread は、実行ユーザー本人、admin、Bot が閲覧できる。
- `/leaderboard` と `/leaderboard_season` の結果は、このチャンネル配下のランキング確認 thread に集約する。
- ランキング確認 thread の詳細仕様は [leaderboard_thread.md](leaderboard_thread.md) を参照する。
...
- 未登録ユーザーは、`matchmaking_channel`、`matchmaking_news_channel`、`leaderboard_channel`、`system_announcements_channel` には入れない。
- 登録完了後は、登録済みユーザー向け閲覧権限の対象となり、`matchmaking_channel`、`matchmaking_news_channel`、`leaderboard_channel`、`system_announcements_channel` を閲覧できる。
...
- ランキング確認 thread UI の詳細は [leaderboard_thread.md](leaderboard_thread.md) を参照する。
